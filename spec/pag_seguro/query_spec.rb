...
describe PagSeguro::Query do
  ...
  describe "instance" do
    ...
    let(:transaction){ PagSeguro::Query.new("trans_code") }
  ...
  describe "::find" do
    it "raises error without valid credentials" do
      expect { PagSeguro::Query.find  }.to raise_error(RestClient::Exception, "401 Unauthorized")
    end
    ...
    context "with abandoned set to true" do
      it "should try to fetch abandoned transactions" do
        ...
        PagSeguro::Query.find abandoned: true
      end
    end

    context "with a stubbed response" do
      before { RestClient.stub(get: transactions_xml) }
      subject{ PagSeguro::Query.find options: true }

      it "calls search_url with received options" do
        PagSeguro::Query.should_receive(:search_params).with({options: true})
        PagSeguro::Query.find options: true
      end
  ...
  describe "::search_params" do
    it("returns an Hash"){ PagSeguro::Query.search_params().should be_a Hash }

    context "with default options" do
      let(:search_params) { PagSeguro::Query.search_params() }
    ...
    context "with page number set to 2" do
      let(:search_params){ PagSeguro::Query.search_params(page: 2) }
      it { search_params[:page].should == 2 }
    end

    context "with max_page_results set to 100" do
      let(:search_params){ PagSeguro::Query.search_params(max_page_results: 100) }
      it { search_params[:maxPageResults].should == 100 }
    end

    context "with initial_date set to 20 days ago" do
      let(:search_params){ PagSeguro::Query.search_params(initial_date: 20.days.ago) }
      it { search_params[:initialDate].should include 20.days.ago.iso8601 }
    end

    context "with final_date set to 10 days ago" do
      let(:search_params){ PagSeguro::Query.search_params(initial_date: 20.days.ago, final_date: 10.days.ago) }
